...
import 'dart:async';
import 'dart:io';
import 'package:customfit_flutter_client/customfit_flutter_sdk.dart';
...
void setupListeners(CFClient cfClient) {
  ...
  cfClient.listenerManager.registerFeatureFlagListener('test-flag', _featureFlagChangeListener);
...
Future<void> testFeatureFlags(CFClient cfClient) async {
  ...
  // Get a string flag
  final stringFlag = cfClient.configManager.getString('string-flag', 'default-value');
  ...
  // Get a JSON flag
  final jsonFlag = cfClient.configManager.getJson('json-flag', {'default': true});
...
Future<void> testEventTracking(CFClient cfClient) async {
  ...
  print('Simple event tracking result: ${result1.isSuccess ? 'Success' : 'Failed'}');
  
  // Track an event with properties
  final result2 = await cfClient.trackEvent('test_event_with_props', properties: {
    'category': 'test',
    'value': 123,
    'items': ['item1', 'item2'],
  });
  print('Event with properties tracking result: ${result2.isSuccess ? 'Success' : 'Failed'}');
...
void _connectionStatusListener(ConnectionStatus status, ConnectionInformation info) {
  print('Connection status changed: $status');
  print('Connection info: ${info.isOfflineMode ? 'Offline' : 'Online'}, failures: ${info.failureCount}');
}
