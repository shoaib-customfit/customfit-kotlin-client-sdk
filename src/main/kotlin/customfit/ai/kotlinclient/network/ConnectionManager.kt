...
/**
 * Manages connection status monitoring and automatic reconnection logic.
 * Platform-agnostic implementation that relies on connection attempts outcomes
 * rather than direct network monitoring.
 */
class ConnectionManager(
     ...
     */
    fun getConnectionInformation(): ConnectionInformation = ConnectionInformation(
            ...
            override fun run() {
                if (!isOfflineMode.get() && 
                    (currentStatus == ConnectionStatus.DISCONNECTED || 
                     System.currentTimeMillis() - lastSuccessfulConnection.get() > 60000)) {
                    checkConnection()
                }
            }
        }
            ...
            scope.launch(Dispatchers.Default) {
                for (listener in listeners) {
                    try {
                        ...
                        logger.error(e) { "Error notifying connection listener" }
                    }
                }
            }
        }
